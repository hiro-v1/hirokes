...
async def tambah_admin(event):
    ...
    if event.sender_id != OWNER_ID:
        return await event.respond("❌ Anda tidak memiliki izin untuk menggunakan perintah ini, bilang ke bang @hiro_v1.")
...
async def message_handler(event):
    ...
    if user_id in banned_users:
        await event.delete()
        logging.info(f"🚫 Pesan dari {user_id} dihapus karena pengguna ini alay.")
        return
    ...
    if await check_message(text) or contains_restricted_chars(text):
        await event.delete()
        await event.respond("⚠️ **Pesan lu alay.**")
